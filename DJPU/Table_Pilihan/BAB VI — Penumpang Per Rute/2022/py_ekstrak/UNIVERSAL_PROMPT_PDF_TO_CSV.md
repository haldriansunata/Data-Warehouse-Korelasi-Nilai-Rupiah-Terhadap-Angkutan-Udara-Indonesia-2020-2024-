...
### Case 6: Route Column Treated as String (Not Number)
- Kolom rute (kolom ke-2) berisi string seperti `"Jakarta (CGK) - Denpasar (DPS)"` yang tidak boleh di-cleaning sebagai angka
- Jika fungsi cleaning number dipaksa ke kolom ini, hasilnya jadi `None` karena bukan angka
- Solusi: Skip kolom rute (kolom ke-2) dari `clean_number()` function, treat langsung sebagai string

### Case 7: Errant Spaces Inside Numbers from PDF Artifacts
- PDF menghasilkan spasi di posisi tidak standar dalam angka: `"7 6.081"` atau `"2 .002.789"` atau `"4 8.418"`
- Spasi ini bukan pemisah ribuan biasa, tapi artifact dari rendering PDF
- Solusi: Hapus **semua** spasi dalam string angka terlebih dahulu, baru hapus titik (pemisah ribuan), lalu konversi ke integer

### Case 8: Total Row with None/Empty Route Column
- Baris agregat "Total" di akhir PDF memiliki kolom rute kosong (`None` atau `""`)
- Ini expected behavior karena baris Total adalah agregat, bukan rute spesifik
- Solusi: Pertahankan baris Total di CSV, biarkan kolom route kosong/NaN
