...
df = monthly_pax(filter_kategori="DOMESTIK")
...
with open(OUT / "metrics.txt", "w", encoding="utf-8") as f:
    f.write("Bab 2 / WS3 — Kurs × Penumpang DOMESTIK\n")
    f.write(f"n = {res['n']}\n")
    f.write(f"slope = {res['slope']:,.2f}\n")
...
ax.set_ylabel("Penumpang DOM (juta)")
ax.set_title(f"Kurs × Penumpang DOMESTIK  |  R²={res['r2']:.3f}, p={res['p']:.3e}")
